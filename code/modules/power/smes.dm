...
/obj/machinery/power/smes/attackby(obj/item/I, mob/user)
	...
	if(istype(I, /obj/item/weapon/wirecutters) && terminal && panel_open)
		...
		if(do_after(user, 50))
			if (prob(50) && electrocute_mob(usr, terminal.powerNode, terminal)) //animate the electrocution if uncautious and unlucky
				var/datum/effect/effect/system/spark_spread/s = new /datum/effect/effect/system/spark_spread
				s.set_up(5, 1, src)
				s.start()
				return
			...
			//FOLIX TODO: turn off node? inputting = 0 //stop inputting, since we have don't have a terminal anymore
			qdel(terminal)
			return
